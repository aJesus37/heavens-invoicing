...
func TestTelegramSendInvoice(t *testing.T) {
	...
	if pixMsg.chatID != "12345" {
		t.Fatalf("pix chat ID: want %q, got %q", "12345", pixMsg.chatID)
	}
...
func TestTelegramPIXKeyPrecedence(t *testing.T) {
	...
	t.Run("reminder uses invoice key over fallback", func(t *testing.T) {
		...
		if len(api.calls) != 2 {
			t.Fatalf("want 2 calls (label + key), got %d", len(api.calls))
		}
		assertContains(t, api.calls[0].text, "Chave PIX:")
		assertContains(t, api.calls[1].text, invoicePix)
